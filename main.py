...
async def update_cmd(client, message):
    """Comando para actualizar desde GitHub sin tocar Putty"""
    msg = await message.reply("🔄 **Buscando actualizaciones en GitHub...**")
    try:
        out = subprocess.check_output(["git", "pull"]).decode("utf-8")
        if "Already up to date" in out:
            await msg.edit("✅ **El bot ya está actualizado.**")
        else:
            await msg.edit(f"📥 **Cambios detectados:**\n`{out}`\n\n⌛ **Reiniciando bot...**")
            # Reiniciar el proceso de Python
            os.execl(sys.executable, sys.executable, *sys.argv)
    except Exception as e:
        await msg.edit(f"❌ **Error al actualizar:** `{e}`")
...
async def cb_handler(client, query):
    ...
    if data == "run":
        ...
        await query.answer("🛑 Cancelado.", show_alert=True)
        await query.message.edit("❌ **Abortado.**")
        cleanup(uid)
...
async def main_startup():
    ...
    print("✅ Bot encendido y con comando Update listo!")
