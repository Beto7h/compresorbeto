...
async def download_ytl(url, custom_name, msg, uid):
    ...
    try:
        ...
        if os.path.exists(filename):
            ...
            raise Exception("No se generó el archivo.")
    except Exception as e:
...
async def process_logic(uid, msg, settings, mode):
    ...
    try:
        ...
        if os.path.exists(output_path):
            last_update_time[uid] = 0
            await app.send_video(chat_id=uid, video=output_path, duration=int(get_duration(output_path)), thumb=generate_thumbnail(output_path, uid), file_name=raw_name, supports_streaming=True, caption=f"✅ **Procesado**", progress=progress_bar, progress_args=(msg, uid, "SUBIENDO A TG"))
...
def get_config_summary(uid):
    ...
    return (f"📝 **CONFIG:**\n💎 **Calidad:** `{s['q_label']}` | 📏 **Res:** `{s['res']}p`\n⚡ **Preset:** `{s['v_label']}` | 🎵 **Audio:** `{s['a_label']}`")
...
def get_settings_menu(uid):
    s = user_settings.get(uid, DEFAULT_SETTINGS)
    return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ VOLVER", callback_data="menu_main")]])
...
async def ytl_handler(client, message):
    ...
    if not text: return await message.reply("⚠️ Uso: `/ytl [url]`")
    url = text.split(" -n ")[0]
    name = text.split(" -n ")[1] if " -n " in text else None
    s_msg = await message.reply("⏳ **YTL:** Iniciando...")
